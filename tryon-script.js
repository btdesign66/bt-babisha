...
function getProductTypeImage(type) {
    // Prefer the full product list from `new_products.js` (sampleFabrics).
    // This ensures dropdown selection also supports "all dresses".
    const list =
        (typeof sampleFabrics !== 'undefined' && Array.isArray(sampleFabrics))
            ? sampleFabrics
            : [];

    if (list.length === 0) return '';

    // Your product list currently uses `category: "lehenga"` for most items.
    // For other dropdown types, fall back to the first available product.
    const normalizedType = String(type || '').toLowerCase();

    const match =
        list.find((p) => String(p.category || '').toLowerCase().includes(normalizedType)) ||
        (normalizedType === 'lehenga' ? list.find((p) => String(p.category || '').toLowerCase().includes('lehenga')) : null) ||
        list[0];
...
function toSafeUrl(url) {
    // Dress image paths contain spaces (e.g. "002. TAARANI-A/...").
    // encodeURI keeps slashes but escapes spaces so fetch() can load them.
    try {
...
function updateSelectedProductDisplay() {
    if (state.selectedProduct) {
        ...
        elements.selectedProductImage.src = toSafeUrl(state.selectedProduct.image || state.selectedProduct.imageUrl);
...
async function ensureSelectedProductImageFile() {
    if (!state.selectedProduct) return;
    if (state.selectedProduct.imageFile) return;

    const imgUrl = state.selectedProduct.image || state.selectedProduct.imageUrl;
    if (!imgUrl || typeof imgUrl !== 'string') return;

    // Convert selected dress image URL -> File expected by multer backend.
    // This makes generation use the exact chosen dress.
    const res = await fetch(toSafeUrl(imgUrl));
    if (!res.ok) {
        throw new Error(`Could not load selected dress image: ${res.status} ${res.statusText}`);
    }
    ...
    const urlName = imgUrl.split('/').pop() || `dress.${ext}`;
    const fileName = urlName.includes('.') ? urlName : `dress.${ext}`;

    state.selectedProduct.imageFile = new File([blob], fileName, { type: mime });
}
...
async function generateTryOn() {
    ...
    try {
        // Convert the selected dress image URL -> File (multer expects a file upload).
        await ensureSelectedProductImageFile();
...
async function fetchProducts() {
    // `new_products.js` defines `sampleFabrics`.
    // We use it directly to show *all* dresses in the modal.
    const list =
        (typeof sampleFabrics !== 'undefined' && Array.isArray(sampleFabrics))
            ? sampleFabrics
            : [];
    ...
    if (list.length === 0) {
        throw new Error('Product list not found. Make sure new_products.js is loaded before tryon-script.js.');
    }

    // Normalize shape expected by the UI grid.
    return list.map((p, idx) => ({
        id: p.id ?? idx + 1,
        name: p.name || `Product ${p.id ?? idx + 1}`,
        price: p.price ?? 0,
        image: p.image,
        type: p.category || 'lehenga',
        category: p.category || 'lehenga',
    }));
}
