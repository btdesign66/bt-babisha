...
async function imageUrlToFile(imageUrl, filenamePrefix = 'dress') {
    // Convert an image URL (relative to this site) into a File so we can upload it via Multer.
    ...
    if (!response.ok) {
        throw new Error(`Failed to load dress image from: ${imageUrl}`);
    }

    const blob = await response.blob();
    if (!blob) {
        throw new Error('Failed to read dress image content.');
    }

    if (blob.size > MAX_FILE_SIZE) {
        throw new Error('Selected dress image is too large. Please try a different one.');
    }
...
async function generateTryOn() {
    ...
    try {
        ...
        if (state.selectedProduct.imageFile) {
            // If the product was already represented as a File, upload it directly.
            ...
            // For all dress cards from the catalog, we must upload the actual dress image.
            const imageUrl = state.selectedProduct.imageUrl || state.selectedProduct.image;
            formData.append('productImage', await imageUrlToFile(imageUrl, 'dress'));
        }
...
async function fetchProducts() {
    // Use the catalog loaded by `new_products.js` (window.sampleFabrics).
    const fabrics = window.sampleFabrics || [];

    if (!Array.isArray(fabrics) || fabrics.length === 0) return [];

    // Map into the minimal shape we need for the product picker modal.
    // We intentionally keep the object small to avoid heavy inline payloads.
    return fabrics
        .filter((p) => p && p.image)
        .map((p) => ({
            id: p.id,
            ...
            price: p.price ?? 0,
            image: p.image,
            type: p.type || p.category || 'lehenga',
            category: p.category || p.type || 'lehenga',
        }));
}
...
function displayProductsInGrid(products) {
    ...
    products.forEach(product => {
        ...
        card.className = 'product-select-item card h-100';
        card.style.cursor = 'pointer';

        const safeImg = product.image;
        card.innerHTML = `
            <img src="${safeImg}" class="card-img-top" alt="${product.name}">
            <div class="card-body">
        ...
        card.addEventListener('click', () => {
            // `selectProductFromGrid` is attached to `window`, so call via `window.`.
            window.selectProductFromGrid(product);
        });

        col.appendChild(card);
        elements.productGrid.appendChild(col);
    });
}
