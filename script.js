...
let itemsPerPage = 6;
const PRODUCT_LISTING_STATE_KEY = 'babishaProductListingState';
...
async function initializeProductsPage() {
    ...
    const hasSavedState = restoreProductListingState();
    ...
    if (hasSavedState) {
        console.log('Restored previous products page state');
...
function createFabricCard(fabric) {
    ...
    // Check wishlist and cart state (defensively)
    const active = isProductInWishlist(fabric.id);
    const heartIconClass = active ? 'fas fa-heart' : 'far fa-heart';
    const heartActiveClass = active ? 'active' : '';
    
    const cartItem = babishaCart.find(item => item.id === fabric.id);
    let cartActionHtml = '';
    
    if (cartItem) {
        cartActionHtml = `
            <div class="d-flex justify-content-between align-items-center w-100">
                <button class="btn btn-outline-primary btn-sm" onclick="viewFabricDetails(${fabric.id})" style="width: 48%; padding: 6px 12px; border-radius: 50px;">
                    <i class="fas fa-eye me-1"></i>Details
                </button>
                <div class="cart-quantity-selector" style="width: 48%; justify-content: space-between;">
                    <button onclick="updateCartQuantity(event, ${fabric.id}, -1)">
        ...
        cartActionHtml = `
            <div class="d-flex justify-content-between align-items-center w-100">
                ...
                <button class="btn btn-primary btn-sm" onclick="addToCartDirect(event, ${fabric.id})" style="width: 48%; padding: 6px 12px; border-radius: 50px;">
    ...
    col.innerHTML = `
        <div class="fabric-card h-100">
            <div class="fabric-image position-relative">
                ${savingsText}
                <button class="wishlist-heart-btn ${heartActiveClass}" data-id="${fabric.id}" onclick="toggleWishlist(event, ${fabric.id})" aria-label="Add to wishlist">
...
function showVirtualTryOnComingSoon() {
    // Redirect to the Virtual Try-On page.
    // If you later want to preselect a product, we can pass query params from the product detail page.
    window.location.href = 'virtual-tryon.html';
}
...
function saveProductListingState() {
    ...
    const listingState = {
        ...
        sortValue: sortSelect ? sortSelect.value : 'featured',
    ...
    try {
        sessionStorage.setItem(PRODUCT_LISTING_STATE_KEY, JSON.stringify(listingState));
    } catch (error) {
        console.warn('Unable to save product listing state:', error);
    }
}

function restoreProductListingState() {
    if (getCurrentPage() !== 'products') {
        return false;
    }

    try {
        const rawState = sessionStorage.getItem(PRODUCT_LISTING_STATE_KEY);
        if (!rawState) {
            return false;
        }

        const listingState = JSON.parse(rawState);
        if (!listingState || listingState.path !== window.location.pathname || listingState.search !== window.location.search) {
            return false;
        }
        ...
        if (searchInput) {
            searchInput.value = listingState.searchTerm || '';
        }

        if (sortSelect) {
            sortSelect.value = listingState.sortValue || 'featured';
        }

        if (minPriceInput && listingState.minPrice !== undefined) {
            minPriceInput.value = listingState.minPrice;
        }

        if (maxPriceInput && listingState.maxPrice !== undefined) {
            maxPriceInput.value = listingState.maxPrice;
        }

        if (priceRangeSlider && listingState.maxPrice !== undefined && listingState.maxPrice !== '') {
            priceRangeSlider.value = listingState.maxPrice;
        }

        if (categoryInputs.length > 0) {
            const selectedCategories = Array.isArray(listingState.selectedCategories) && listingState.selectedCategories.length > 0
                ? listingState.selectedCategories
                : ['all'];
        ...
        if (listingState.searchTerm) {
            handleSearch();
        }

        if (listingState.sortValue && listingState.sortValue !== 'featured') {
            applySorting();
        }

        currentPageNumber = listingState.currentPageNumber || 1;
        displayFabrics();

        window.requestAnimationFrame(() => {
            window.scrollTo(0, listingState.scrollY || 0);
        });

        sessionStorage.removeItem(PRODUCT_LISTING_STATE_KEY);
        return true;
    } catch (error) {
        console.warn('Unable to restore product listing state:', error);
        sessionStorage.removeItem(PRODUCT_LISTING_STATE_KEY);
        return false;
    }
}
...
// Redirect to HDFC SmartGateway payment page (payment.html) with prefilled amount/product data.
function goToHdfcPaymentFromDetailPage() {
    const urlParams = new URLSearchParams(window.location.search);
    ...
    const amount = fabric.price;
    const productName = fabric.name;

    window.location.href =
        `payment.html?amount=${encodeURIComponent(amount)}&productName=${encodeURIComponent(productName)}&productId=${encodeURIComponent(productId)}`;
}

window.goToHdfcPaymentFromDetailPage = goToHdfcPaymentFromDetailPage;
...
/* ==========================================================================
   BABISHA - Cart & Wishlist System Implementation
   ========================================================================== */

// Globals for Cart & Wishlist
let babishaCart = [];
let babishaWishlist = [];
let cartOffcanvasInstance = null;
let wishlistOffcanvasInstance = null;

// Initialize Cart and Wishlist
function initCartAndWishlist() {
    console.log('Initializing Cart & Wishlist state...');
    
    // Load from localStorage
    try {
        const storedCart = localStorage.getItem('babisha_cart');
        if (storedCart) {
            babishaCart = JSON.parse(storedCart);
            console.log('Loaded cart:', babishaCart.length, 'items');
        }
    } catch (e) {
    ...
    try {
        const storedWishlist = localStorage.getItem('babisha_wishlist');
        if (storedWishlist) {
            babishaWishlist = JSON.parse(storedWishlist);
            console.log('Loaded wishlist:', babishaWishlist.length, 'items');
        }
    } catch (e) {
    ...
    // Inject Offcanvas elements and toast container
    injectCartAndWishlistDrawers();
    
    // Setup navbar buttons to open drawers
    setupNavbarBadges();
    ...
    // Update product card displays if we are on products page
    if (getCurrentPage() === 'products') {
        // Redraw grid to reflect current cart quantities and wishlist hearts
        setTimeout(() => {
            if (typeof displayFabrics === 'function') {
                displayFabrics();
            }
        }, 100);
    }
    
    // Update product details page if on details page
    if (window.location.pathname.includes('product-detail.html')) {
        updateProductDetailButtons();
    }
}

// Inject drawers and toast containers to body
function injectCartAndWishlistDrawers() {
    // 1. Toast Container
    if (!document.getElementById('babishaToastContainer')) {
        const toastContainer = document.createElement('div');
        toastContainer.id = 'babishaToastContainer';
        toastContainer.className = 'babisha-toast-container';
        ...
        const cartDrawerHtml = `
            <div class="offcanvas offcanvas-end" tabindex="-1" id="cartDrawer" aria-labelledby="cartDrawerLabel">
                ...
                <div class="offcanvas-body d-flex flex-column h-100">
                    <div id="cartItemsList" class="flex-grow-1 overflow-auto" style="max-height: calc(100vh - 280px);">
                        <!-- Cart items will be loaded here -->
                    </div>
                    <div id="cartSummaryContainer">
                        <!-- Summary and WhatsApp checkout will be loaded here -->
                    </div>
                </div>
            </div>
        `;
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = cartDrawerHtml.trim();
    ...
    if (!document.getElementById('wishlistDrawer')) {
        const wishlistDrawerHtml = `
            <div class="offcanvas offcanvas-end" tabindex="-1" id="wishlistDrawer" aria-labelledby="wishlistDrawerLabel">
                <div class="offcanvas-header">
                    <h5 class="offcanvas-title" id="wishlistDrawerLabel">
                        <i class="fas fa-heart me-2"></i>My Wishlist
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                </div>
                <div class="offcanvas-body d-flex flex-column h-100">
                    <div id="wishlistItemsList" class="flex-grow-1 overflow-auto">
        ...
        `;
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = wishlistDrawerHtml.trim();
        document.body.appendChild(tempDiv.firstChild);
    }
}
...
function showToast(message, type = 'cart') {
    const container = document.getElementById('babishaToastContainer');
    if (!container) return;

    const toast = document.createElement('div');
    toast.className = `babisha-toast ${type}-toast`;
    
    const icon = type === 'cart' ? 'fa-shopping-cart' : 'fa-heart';
    toast.innerHTML = `
        <div class="babisha-toast-icon">
            <i class="fas ${icon}"></i>
        </div>
        <div class="babisha-toast-message">${message}</div>
    `;
...
// Update Navbar Badges & Event Handlers
function setupNavbarBadges() {
    const navLinks = document.querySelectorAll('.navbar-nav .nav-link');
    
    navLinks.forEach(link => {
        const onclickAttr = link.getAttribute('onclick');
        if (onclickAttr && onclickAttr.includes('openCart')) {
            // Setup Cart Badge
            link.removeAttribute('onclick');
            link.className = 'nav-link nav-badge-container';
            link.innerHTML = `
                <i class="fas fa-shopping-cart"></i>
                <span class="d-lg-none ms-2">Cart</span>
                <span class="nav-badge" id="cartNavCount">0</span>
            `;
        ...
        if (onclickAttr && onclickAttr.includes('openWishlist')) {
            // Setup Wishlist Badge
            link.removeAttribute('onclick');
            link.className = 'nav-link nav-badge-container';
            link.innerHTML = `
                <i class="fas fa-heart"></i>
                <span class="d-lg-none ms-2">Wishlist</span>
                <span class="nav-badge" id="wishlistNavCount">0</span>
            `;
            link.addEventListener('click', (e) => {
                e.preventDefault();
                toggleWishlistDrawer(true);
...
// Toggle drawers programmatically using Bootstrap 5 Offcanvas API
function toggleCartDrawer(open = true) {
    const cartEl = document.getElementById('cartDrawer');
    if (!cartEl) return;
    
    if (!cartOffcanvasInstance) {
        cartOffcanvasInstance = new bootstrap.Offcanvas(cartEl);
    }
...
function toggleWishlistDrawer(open = true) {
    ...
    if (!wishlistOffcanvasInstance) {
        wishlistOffcanvasInstance = new bootstrap.Offcanvas(wishlistEl);
    }
    
    if (open) {
        // Close cart if open
        toggleCartDrawer(false);
        wishlistOffcanvasInstance.show();
    } else {
        wishlistOffcanvasInstance.hide();
    }
}

// Update Cart Badge Count and Drawer HTML Content
function updateCartUI() {
    const cartCountEl = document.getElementById('cartNavCount');
    const totalItems = babishaCart.reduce((total, item) => total + item.quantity, 0);
    ...
    const cartSummaryEl = document.getElementById('cartSummaryContainer');
    if (!cartListEl) return;

    if (babishaCart.length === 0) {
        cartListEl.innerHTML = `
            <div class="empty-drawer-view">
                <i class="fas fa-shopping-cart"></i>
                <h5>Your cart is empty</h5>
                <p class="text-muted">Browse our products to add items to your cart.</p>
                <a href="products.html" class="btn btn-primary btn-sm rounded-pill mt-3" data-bs-dismiss="offcanvas">
                    Shop Fabrics
                </a>
            </div>
        `;
            ...
            <div class="drawer-item">
                ...
                <div class="drawer-item-info">
                    <div>
                        ...
                        <div class="drawer-item-price">₹${item.price.toLocaleString()}/piece</div>
                    </div>
                    <div class="drawer-item-actions">
                        <div class="cart-quantity-selector">
                            <button onclick="updateCartQuantity(event, ${item.id}, -1)">
                                <i class="fas fa-minus"></i>
                            </button>
                        ...
                        <button class="drawer-item-remove" onclick="removeCartItemDirect(${item.id})">
    ...
    if (cartSummaryEl) {
        cartSummaryEl.innerHTML = `
            <div class="drawer-summary">
                ...
                <button class="btn btn-primary checkout-btn mb-2 w-100" onclick="proceedToOrder(${subtotal})">
                    <i class="fas fa-credit-card me-2"></i>Proceed to Order
                </button>
                <button class="btn btn-warning checkout-btn w-100" onclick="checkoutWhatsApp()">
...
// Update Wishlist Badge Count and Drawer HTML Content
function updateWishlistUI() {
    const wishlistCountEl = document.getElementById('wishlistNavCount');
    const totalItems = babishaWishlist.length;
    
    if (wishlistCountEl) {
        wishlistCountEl.textContent = totalItems;
        wishlistCountEl.style.display = totalItems > 0 ? 'flex' : 'none';
    }

    const wishlistListEl = document.getElementById('wishlistItemsList');
    if (!wishlistListEl) return;

    if (babishaWishlist.length === 0) {
        wishlistListEl.innerHTML = `
            <div class="empty-drawer-view">
                <i class="far fa-heart"></i>
                <h5>Your wishlist is empty</h5>
                <p class="text-muted">Tap the heart icon on any fabric to save it here.</p>
                <a href="products.html" class="btn btn-primary btn-sm rounded-pill mt-3" data-bs-dismiss="offcanvas">
                    Browse Fabrics
                </a>
            </div>
            ...
            <div class="drawer-item">
                ...
                <div class="drawer-item-info">
                    ...
                    <div class="drawer-item-actions mt-2">
                        <button class="btn btn-primary btn-sm px-3 rounded-pill" onclick="moveFromWishlistToCart(${item.id})">
                            <i class="fas fa-shopping-cart me-1"></i>Add to Cart
                        </button>
                        <button class="drawer-item-remove ms-2" onclick="toggleWishlist(event, ${item.id})">
                            <i class="fas fa-trash-alt"></i>
                        </button>
                    </div>
                </div>
    ...
    wishlistListEl.innerHTML = listHtml;
}

// Helper to check if a product is in wishlist
function isProductInWishlist(id) {
    return babishaWishlist.some(item => item.id === id);
}
...
function toggleWishlist(event, id) {
    ...
    const allFabrics = (typeof fabricData !== 'undefined' && fabricData.length > 0) ? fabricData : (sampleFabrics || []);
    const fabric = allFabrics.find(f => f.id === id);
    if (!fabric) return;

    const index = babishaWishlist.findIndex(item => item.id === id);
    let message = '';
    
    if (index !== -1) {
        // Remove from wishlist
        babishaWishlist.splice(index, 1);
        message = 'Removed from Wishlist';
    } else {
        // Add to wishlist
        babishaWishlist.push({
            id: fabric.id,
            name: fabric.name,
            price: fabric.price,
            image: fabric.image
    ...
    // Save
    localStorage.setItem('babisha_wishlist', JSON.stringify(babishaWishlist));
    updateWishlistUI();
    showToast(message, 'wishlist');

    // Update active hearts on cards
    updateHeartButtonStates(id);
    
    // Update product detail page if active
    if (window.location.pathname.includes('product-detail.html')) {
        updateProductDetailButtons();
    }
}

// Update heart buttons active state in the DOM
function updateHeartButtonStates(id) {
    const heartBtns = document.querySelectorAll(`.wishlist-heart-btn[data-id="${id}"]`);
...
// Add directly from card button click
function addToCartDirect(event, id) {
    if (event) {
        event.preventDefault();
        event.stopPropagation();
    }
    ...
    const existingItem = babishaCart.find(item => item.id === id);
    if (existingItem) {
    ...
    // Save
    localStorage.setItem('babisha_cart', JSON.stringify(babishaCart));
    updateCartUI();
    showToast('Added to Cart!', 'cart');

    // Redraw grid or update this specific card actions to show counter
    if (typeof displayFabrics === 'function') {
        displayFabrics();
    }
    
    // Update detail page actions if active
    if (window.location.pathname.includes('product-detail.html')) {
        updateProductDetailButtons();
    }
...
function updateCartQuantity(event, id, delta) {
    if (event) {
        event.preventDefault();
        event.stopPropagation();
    }

    const cartItem = babishaCart.find(item => item.id === id);
    if (!cartItem) return;

    cartItem.quantity += delta;

    if (cartItem.quantity <= 0) {
        const index = babishaCart.indexOf(cartItem);
        babishaCart.splice(index, 1);
    ...
    // Redraw grid or update cards to show new quantity
    if (typeof displayFabrics === 'function') {
        displayFabrics();
    }
...
function removeCartItemDirect(id) {
    const index = babishaCart.findIndex(item => item.id === id);
    if (index !== -1) {
        babishaCart.splice(index, 1);
        localStorage.setItem('babisha_cart', JSON.stringify(babishaCart));
        updateCartUI();
...
// Move from Wishlist to Cart Drawer
function moveFromWishlistToCart(id) {
    // 1. Remove from Wishlist
    const wlIndex = babishaWishlist.findIndex(item => item.id === id);
    if (wlIndex !== -1) {
        babishaWishlist.splice(wlIndex, 1);
        localStorage.setItem('babisha_wishlist', JSON.stringify(babishaWishlist));
        updateWishlistUI();
        updateHeartButtonStates(id);
    }

    // 2. Add to Cart (will save, update cart UI, redraw cards and open cart drawer)
    addToCartDirect(null, id);
}

// Structured Checkout via WhatsApp
function checkoutWhatsApp() {
    if (babishaCart.length === 0) return;

    let message = "Hi! I'd like to place an order for the following items from *BABISHA*:\n\n";
    let subtotal = 0;

    babishaCart.forEach((item, index) => {
        const itemTotal = item.price * item.quantity;
        subtotal += itemTotal;
        message += `${index + 1}. *${item.name}*\n`;
        message += `   Qty: ${item.quantity} piece(s) | Price: ₹${item.price.toLocaleString()}/piece\n`;
        message += `   Total: ₹${itemTotal.toLocaleString()}\n\n`;
    });

    message += `*Total Order Value:* *₹${subtotal.toLocaleString()}*\n\n`;
    message += "Please confirm my order and share the availability and payment details. Thank you!";

    const whatsappUrl = `https://wa.me/917567114541?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, '_blank');
}

// Proceed to Order (redirects to payment.html with cart details)
function proceedToOrder(amount) {
    if (babishaCart.length === 0) return;
    const productNames = babishaCart.map(item => `${item.name} (x${item.quantity})`).join(", ");
    const productName = "Cart Order - " + productNames;
    const productId = "cart";
    window.location.href = `payment.html?amount=${encodeURIComponent(amount)}&productName=${encodeURIComponent(productName)}&productId=${encodeURIComponent(productId)}`;
}
window.proceedToOrder = proceedToOrder;

// Product Details Page Button Upgrades
function updateProductDetailButtons() {
    const urlParams = new URLSearchParams(window.location.search);
    const productId = parseInt(urlParams.get('id'));
    if (!productId) return;

    // 1. Update Favorites Button UI
    const favBtn = document.querySelector('.action-buttons button[onclick="addToFavorites()"]');
    if (favBtn) {
        const active = isProductInWishlist(productId);
        if (active) {
            favBtn.className = 'btn btn-primary btn-lg me-3';
            favBtn.innerHTML = '<i class="fas fa-heart me-2"></i>In Favorites';
        } else {
            favBtn.className = 'btn btn-outline-primary btn-lg me-3';
            favBtn.innerHTML = '<i class="far fa-heart me-2"></i>Add to Favorites';
        }
    }

    // 2. Add Cart Quantity / Add-to-Cart Controls to Action Buttons
    const actionButtonsContainer = document.querySelector('.action-buttons');
    if (!actionButtonsContainer) return;

    // Check if we already injected our cart row
    let detailCartRow = document.getElementById('detailCartRow');
    if (!detailCartRow) {
        detailCartRow = document.createElement('div');
        detailCartRow.id = 'detailCartRow';
        detailCartRow.className = 'd-flex align-items-center mt-3';
        actionButtonsContainer.appendChild(detailCartRow);
    }

    const cartItem = babishaCart.find(item => item.id === productId);
    if (cartItem) {
        detailCartRow.innerHTML = `
            <span class="me-3 fw-bold text-dark">In Cart:</span>
            <div class="cart-quantity-selector" style="padding: 4px 8px;">
                <button onclick="updateCartQuantity(event, ${productId}, -1)">
                    <i class="fas fa-minus"></i>
                </button>
                <span class="mx-3 fw-bold" style="font-size: 1.1rem;">${cartItem.quantity}</span>
                <button onclick="updateCartQuantity(event, ${productId}, 1)">
                    <i class="fas fa-plus"></i>
                </button>
            </div>
            <button class="btn btn-outline-danger btn-sm ms-3" onclick="removeCartItemDirect(${productId})">
                <i class="fas fa-trash-alt me-1"></i>Remove
            </button>
        `;
    } else {
        detailCartRow.innerHTML = `
            <button class="btn btn-primary btn-lg" onclick="addToCartDirect(null, ${productId})">
...
// Maps detail page add to favorites action
...
window.openWishlist = () => toggleWishlistDrawer(true);
